...
This repository is a zsh integration for the Codex CLI. `codex-zsh.zsh` is the entrypoint users source from `.zshrc`. Runtime code lives in `lib/`: `lib/agent.zsh` handles `:`, `:new`, `:plan`, model, effort, session state, and the preexec hook; `lib/commit.zsh` handles `:commit`. The optional Codex commit skill lives at `skills/commit/SKILL.md`. `README.md` is the user-facing install and usage guide. There is no dedicated `test/` directory or asset pipeline.
...
Write zsh, not bash. Prefer `emulate -L zsh` inside functions and use zsh arrays intentionally. Keep agent helpers private with the `_codex_agent_` prefix. Public commands use colon-prefixed names such as `:new`, `:plan`, `:model`, `:effort`, `:commit`, and `:agent-debug`. Use two-space indentation and concise stderr messages with `print -u2 --`.
